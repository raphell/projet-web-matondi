    ...
    <div class="container-fluid">
        ...
        <div class="contact-section">
            <div class="container">
	            ...
	            <% 
	                int nbrEvent = (int)session.getAttribute("nbrAllEvents");
	                for(int i=0 ; i<nbrEvent ; i++){
	                    Evenement event = (Evenement)session.getAttribute("allEvent"+i);
	            %>
	            <div class="row">
	                <form method="get" action="GererEvenement" class="text-center col-xs-12 col-sm-12 col-md-12 col-lg-12">
	                    <div class="form-group">
		                    <input type="radio" style="display:none" name="radio" value="<%=event.getId() %>" checked="checked"></input>
		                    <button type="submit" class="btn btn-default submit" name="submitbutton" value="<%=event.getId() %>" class="btn btn-primary"> <%=event.getName()%> <br/>Date :<%=event.getDate()%></button>
	                    </div>
	                </form>
	            </div>
	            <% } %>
